...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        scores = utils2.batch_slice([scores, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        deltas = utils2.batch_slice([deltas, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        anchors = utils2.batch_slice(ix, lambda x: tf.gather(anchors, x),
                                    self.config.IMAGES_PER_GPU,
                                    names=["pre_nms_anchors"])

        # Apply deltas to anchors to get refined anchors.
        # [batch, N, (y1, x1, y2, x2)]
        boxes = utils2.batch_slice([anchors, deltas],
        ...
        boxes = utils2.batch_slice(boxes,
        ...
        def nms(normalized_boxes, scores):
            ...
            return proposals
        proposals = utils2.batch_slice([normalized_boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        return proposals
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    deltas = utils2.box_refinement_graph(positive_rois, roi_gt_boxes)
...
def detection_keypoint_targets_graph(proposals, gt_class_ids, gt_boxes, gt_keypoints, config):#去掉了输入只gt_masks
    """Generates detection targets for one image. Subsamples proposals and
    generates target class IDs, bounding box deltas, and masks for each.
    ...
    return rois, roi_gt_class_ids, deltas, keypoint_lables, keypoint_weights #去掉了返回值masks
...
class DetectionKeypointTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        names = ["rois", "target_class_ids", "target_bbox", "target_keypoint","target_keypoint_weight"]#去掉了target_mask
        outputs = utils2.batch_slice(
            [proposals, gt_class_ids, gt_boxes, gt_keypoints],          #去掉了gt_masks
            lambda r, x, y, z: detection_keypoint_targets_graph(
                r, x, y, z, self.config),
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    image, window, scale, padding = utils2.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    mask = utils2.resize_mask(mask, scale, padding)
    ...
    bbox = utils2.extract_bboxes(mask)
    ...
    if use_mini_mask:
        mask = utils2.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
def load_image_gt_keypoints(dataset, config, image_id, augment=True,
    ...
    shape = image.shape
    shape_fi = shape[:]
    keypoints, _, class_ids = dataset.load_keypoints(image_id)
    ...
    image, window, scale, padding = utils2.resize_image(
        ...
        padding=config.IMAGE_PADDING)
    #mask = utils2.resize_mask(mask, scale, padding)
    bbox = utils2.extract_fi_bboxes(keypoints,image.shape[:2],shape_fi[:2], scale, padding)
    keypoints = utils2.resize_keypoints(keypoints, image.shape[:2], scale, padding)
    ...
    if augment:
        if random.randint(0, 1):
            image = np.fliplr(image)
            #mask = np.fliplr(mask)
            keypoint_names,keypoint_flip_map = utils2.get_keypoints(config.IMAGE_CATEGORY)

            #print('model.py:1758:',keypoint_names,keypoint_flip_map,keypoints,image.shape[1])
            keypoints = utils2.flip_keypoints(keypoint_names,keypoint_flip_map,keypoints, image.shape[1])
    ...
    # bbox = utils2.extract_bboxes(mask)
    # bbox = utils2.extract_fi_bboxes(keypoints)
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    bboxes[pos_ids, roi_gt_class_ids[pos_ids]] = utils2.box_refinement(
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        # Compute overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = utils2.compute_overlaps(anchors, crowd_boxes)
    ...
    overlaps = utils2.compute_overlaps(anchors, gt_boxes)
...
def data_generator_keypoint(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    anchors = utils2.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
    ...
    while True:
        try:
            ...
            #image_meta:image_id,image_shape,windows.active_class_ids
            image, image_meta, gt_class_ids, gt_boxes, gt_keypoints = \
            ...
            if b >= batch_size:
                inputs = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
                          batch_gt_class_ids, batch_gt_boxes, batch_gt_keypoints]
                outputs = []
                # Not implemented for keypoint_mask and no need here.
                if random_rois:
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Generate Anchors
        self.anchors = utils2.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                                                      config.RPN_ANCHOR_RATIOS,
                                                      config.BACKBONE_SHAPES,
                                                      config.BACKBONE_STRIDES,
                                                      config.RPN_ANCHOR_STRIDE)
        ...
        if mode == "training":
            ...
            model = KM.Model([input_image, input_image_meta],
                             [detections, mrcnn_class, mrcnn_bbox, rpn_rois, rpn_class, rpn_bbox, keypoint_mcrcnn_prob],
    ...
    def get_imagenet_weights(self):
        ...
        """
        from keras.utils2.data_utils2 import get_file
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = ["rpn_class_loss", "rpn_bbox_loss",
                      "mrcnn_class_loss", "mrcnn_bbox_loss", "keypoint_mrcnn_mask_loss"]#去掉mrcnn_mask_loss
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = utils2.resize_image(
                image,
    ...
    def unmold_keypoint_detections(self, detections, mrcnn_keypoints, image_shape, window,keypoint_threshold = 0.05):#去掉mrcnn mask
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        for i in range(N):
            ...
            keypoint = utils2.unmold_keypoint_mask(mrcnn_keypoints[i], boxes[i], image_shape,keypoint_threshold=keypoint_threshold)
        ...
        return boxes, class_ids, scores, keypoints
    ...
    def detect_keypoint(self, images, verbose=0):
        ...
        detections, mrcnn_class, mrcnn_bbox, \
            rois, rpn_class, rpn_bbox, mrcnn_keypoint_prob =\
            self.keras_model.predict([molded_images, image_metas], verbose=0)
        if verbose:
        ...
        results = []
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_keypoints=\
                self.unmold_keypoint_detections(detections[i], mrcnn_keypoint_prob[i],
                                       image.shape, windows[i],keypoint_threshold = self.config.KEYPOINT_THRESHOLD)
            results.append({
                "rois": final_rois,
                "class_ids": final_class_ids,
                "scores": final_scores,
